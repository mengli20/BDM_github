...
#' Soil moisture and temp sensor boxplot (Moisture)
#'
...
#' @details This function takes data from Decagon 5TM soil/moisture sensors and returns a box plot of the soil moisture data for the time period requested 
#' @export 
#' @examples
#' smtsensorboxpm(dfsensor, sensornameg, "2015-08-10", "2015-08-20")

smtsensorboxpm <- function(df, sensornames, startdate, enddate) {
  ...
  dfplot<- df_3 %>%
    ...
    select (3,5,7,9) %>%  #columns containing moisture data 
    gather (sensor, moisture) %>%  #make wide df long 
    group_by(sensor) %>%  
    ggplot(aes(x = sensor, y= moisture)) +
    geom_boxplot()
  return(dfplot)
